...
then
	...
	then
		TITLE=$(zenity --forms --add-combo="" --combo-values="Red Alert|Tiberian Dawn|Dune 2000|Yuris Revenge|Tiberian Sun" --text "Select mod" --title="" || echo "cancel")
		...
		elif [ "$TITLE" = "Yuris Revenge" ]; then MODARG='Game.Mod=ts'
		else MODARG='Game.Mod=ra'
		fi
	else
		echo "Please provide the Game.Mod=\$MOD argument (possible \$MOD values: ra, cnc, d2k, ts, yr)"
		exit 1
	fi
fi
